<script lang="ts">
  ...
  let todayPrediction = $derived(predictions.find((p: any) => p.log_date === new Date().toISOString().split('T')[0]));
...
<div class="page-header">
  <div>
    ...
    <div class="page-subtitle">What's driving today's crash risk · {formatDate(new Date().toISOString().split('T')[0])}</div>
  </div>
